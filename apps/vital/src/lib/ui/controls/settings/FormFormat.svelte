<script lang="ts">
	...
	import {FormFormatValidator} from '$lib/common/validate'
	...
	let {coordMetadata, cta, format = '', color = 'primary'}: Props = $props()

	const validator = new FormValidator(FormFormatValidator)
	...
	let formats = $derived(coordMetadata.getFormats())
	...
	function updateFormat(event: Event) {
		...
		const formatFound = coordMetadata.checkFormatExists(newFormat)
