...
import {blur, defaultFrag} from './shaders/fragment-shader'
import {vert} from './shaders/vertex-shader-3d'

function convertToChannelOrder(str) {
	return str.split('').map((c) => (c === 'r' ? 0 : c === 'a' ? 3 : c === 'g' ? 1 : 2))
}
const channels = {
...
function update(context, {filters}) {
	...
	if (filters.channels && filters.channels !== channelOrder) {
		...
		console.log('channelOrder', channels[channelOrder])
		gl.uniform4iv(
			gl.getUniformLocation(programInfo.program, 'u_channelSwap'),
