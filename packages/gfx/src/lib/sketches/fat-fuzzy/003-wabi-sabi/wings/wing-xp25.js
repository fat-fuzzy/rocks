...
export default class Wing {
	...
	getBoneVertices() {
		...
		for (bone = 0; bone < currentMovement.length - 1; bone++) {
			...
			if (this.drawFeathers && bone > 0) {
				featherVectors = this.getFeatherVertices(bone, magnitude, coords)
				boneVectors.push(...featherVectors)
			}
		}
	 ...
	 */
	getFeatherVertices(bone, magnitude, coords) {
		let featherVectors = []
		let insertionOrigin
		let insertionDistance = 0
		let featherMagnitude = this.magnitudes.feathers[bone - 1].magnitude
		let featherCount = this.magnitudes.feathers[bone - 1].featherCount
		let featherAngles = this.angles.feathers[this.currentTime]
		let featherAngle = featherAngles[bone - 1]

		let distance = magnitude / featherCount
		let unit = vectors.getUnitVector(...coords, magnitude)
