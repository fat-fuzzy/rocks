...
async function main(canvas, glContext) {
	gl = glContext
	// Only continue if WebGL is available and working
	if (gl === null) {
		throw Error(
	...
	loadImages(urls, (loadedImages) => {
		...
		setTextureAttribute(gl, buffers, programInfo, DEFAULT_TEXTURE_COORDS)
		error = gl.getError()
	...
	return Promise.resolve(programInfo.context)
}
...
function loadProgram(canvas) {
	...
	setTextureAttribute(gl, buffers, _programInfo, DEFAULT_TEXTURE_COORDS)
...
function update({texture}) {
	...
	programInfo.context = {
		images,
		translation: [0, 0],
		effects: texture.filters?.effects ?? ['normal'],
		level,
	}
}
