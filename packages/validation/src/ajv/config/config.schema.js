import {NamespaceSchema} from '../in/ajv.schemas.js'

const FatFuzzyConfigSchema = {
		...
		ValidationConfig: {
			...
			properties: {
				...
				schemas: {
					type: 'object',
					description:
						"Per-schema configuration keyed by schema name. Names matching built-ins (Form) may use mode 'extend' or 'replace'. New names are always additive.",
				...
				ajvOptions: {
					type: 'object',
					description: 'AJV init options.',
					properties: {
						removeAdditional: {
							type: 'boolean',
							description: 'If true: strip unknown fields from user imports.',
						},
						coerceTypes: {
							type: 'boolean',
							description: "If true: '123' becomes 123, 'true' becomes true.",
						},
						useDefaults: {
							type: 'boolean',
							description:
								'If true: fill in schema `default` values automatically.',
						},
					},
				},
			},
		...
		SchemaEntry: {
			...
			properties: {
				...
				$id: {
					...
					minLength: 1,
					maxLength: 128,
					pattern: '^(#/definitions/){0,1}[A-Za-z_][A-Za-z0-9_-]*$',
					description:
...
export default FatFuzzyConfigSchema
